...
public class Kanner extends FrameLayout {
	...
	public void setImagesUrl(String[][] imagesUrl) {
		initLayout();
		initImgFromNetWithClick(imagesUrl);
		showTime();
	}
	...
	private void initImgFromNetWithClick(final String[][] imagesUrl) {
		count = imagesUrl.length;
		for (int i = 0; i < count; i++) {
			ImageView iv_dot = new ImageView(context);
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
		...
		for (int i = 0; i <= count + 1; i++) {
			...
			iv.setBackgroundResource(R.drawable.loading);
			if (i == 0) {
				// mImageLoader.displayImage(imagesUrl[count - 1][0], iv);
				bitmapUtils.display(iv, imagesUrl[count - 1][0]);
			} else if (i == count + 1) {
				/// mImageLoader.displayImage(imagesUrl[0][0], iv);
				bitmapUtils.display(iv, imagesUrl[0][0]);
			} else {
				// mImageLoader.displayImage(imagesUrl[i - 1][0], iv);
				bitmapUtils.display(iv, imagesUrl[i - 1][0]);
			}
			iv.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View v) {
					System.out.println("" + position + "----" + imagesUrl);
					String url;
					if (position == 0) {
						url = imagesUrl[count - 1][1];
					} else if (position == count + 1) {
						url = imagesUrl[0][1];
					} else {
						url = imagesUrl[position - 1][1];
					}
