...
public class NetUtils {
	...
	public static void getScanWifiResults(Context context) {
		...
		for (ScanResult wifi : wifiResults) {
			LogUtils.d(wifi.toString());
		}

		TelephonyManager tm = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);

		LogUtils.d(TelephonyManager.PHONE_TYPE_GSM + "----" + tm.getPhoneType());
		List<NeighboringCellInfo> cellResults = tm.getNeighboringCellInfo();
		for (NeighboringCellInfo cell : cellResults) {
			LogUtils.d(cell.getCid() + "-" + cell.getLac() + "-" + cell.getRssi() + "-" + cell.getPsc() + "-"
					+ cell.getNetworkType());
		}

		LogUtils.d(getProviderName(context).getText());
	}
	...
	public static void coonFairException(String arg1, Context mContext) {
		if (NetUtils.getHttpException(arg1).equals("org.apache.http.conn.ConnectTimeoutException"))
			ToastUtil.show(mContext, ErrCodeUtils.NORMAL_LOGIN_TIMEOUT);
		else
			ToastUtil.show(mContext, ErrCodeUtils.NORMAL_LOGIN_NONNETWORK);
	}
